...
/** Especialista LifSure — Demanda. Mesmo princípio dos outros dois: uma única chamada de raciocínio. */
export async function atenderDemandaLifsure({ demandaTexto, usuarioId, organizacaoId, clienteProspectId, imagens = [], casoIdContinuacao = null }) {
  ...
  if (casoExistente) {
    ...
    const codigoCaso = await gerarCodigoDemandaLifsure()
...
export async function encerrarConversaLifsure(casoId) {
...
export async function buscarHistoricoChatLifsure(casoId) {
...
async function gerarCodigoDemandaLifsure() {
  const { data, error } = await operacional.rpc('gerar_codigo_demanda_lifsure')
  if (error) {
    console.warn('Falha ao gerar código sequencial, usando fallback aleatório:', error.message)
    return `DM-LIFSURE-${Math.floor(1000 + Math.random() * 9000)}`
  }
  return data
}
