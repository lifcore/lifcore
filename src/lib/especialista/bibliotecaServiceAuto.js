 ...
 * Especialista de Auto/Frota. Aponta sempre para as tabelas "_auto"
 * (biblioteca_auto, casos_auto), fisicamente separadas das do
 * Especialista de Saúde (que usa bibliotecaService.js).
 *
 * Este arquivo existe separado de propósito — mesmo que o código
 * pareça duplicado, é exatamente isso que garante que um bug de
 * digitação nunca misture conhecimento de um especialista no outro.
 */
...
/** Busca documentos da biblioteca do Auto por categoria, ordenados por relevância ao texto */
export async function buscarBibliotecaRelevanteAuto(categoria, textoDemanda, limite = 5) {
  const { data, error } = await institucional
    .from('biblioteca_auto')
    .select('codigo, titulo, conteudo, categoria')
    .eq('categoria', categoria)
    .limit(50)

  if (error) throw new Error(`Erro ao consultar biblioteca Auto (${categoria}): ${error.message}`)
...
/** Busca um documento da biblioteca do Auto direto pelo código (ex: REL-001, o Relatório de Engenharia Cognitiva) */
export async function buscarDocumentoAutoPorCodigo(codigo) {
  const { data, error } = await institucional
    .from('biblioteca_auto')
  ...
  if (error) {
    console.warn(`Não foi possível buscar o documento ${codigo} da biblioteca Auto:`, error.message)
    return null
  }
  return data
}

/** Busca casos reais do Auto por relevância ao texto da demanda (começa vazio, cresce com o uso) */
export async function buscarCasosRelevantesAuto(textoDemanda, limite = 4) {
  const { data, error } = await institucional
    .from('casos_auto')
  ...
  if (error) throw new Error(`Erro ao consultar casos reais do Auto: ${error.message}`)
  if (!data || data.length === 0) return []
