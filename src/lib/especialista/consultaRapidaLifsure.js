...
import { gerarRespostaEspecialistaLifsure } from './especialistaMotorLifsure'

/** Consulta Rápida do LifSure — mesmo motor de raciocínio único da Demanda, sem ciclo de vida. */
export async function atenderConsultaRapidaLifsure({ demandaTexto, usuarioId, organizacaoId, imagens = [], consultaIdExistente = null }) {
  ...
  if (consultaIdExistente) {
    const { data } = await operacional.from('consultas_rapidas_lifsure').select('*').eq('id', consultaIdExistente).maybeSingle()
    consulta = data
  }
  ...
  if (!consulta) {
    const { data: novaConsulta, error } = await operacional
      .from('consultas_rapidas_lifsure')
  ...
  const resultado = await gerarRespostaEspecialistaLifsure({
    demandaTexto,
...
/** Converte uma Consulta Rápida em Demanda de verdade, vinculando a um cliente do LifSure */
export async function vincularConsultaComoDemandaLifsure({ consultaId, clienteProspectId, organizacaoId, usuarioId }) {
  const organizacaoIdFinal = organizacaoId ?? (await buscarOrganizacaoUnica())
  ...
  const { data: codigo } = await operacional.rpc('gerar_codigo_demanda_lifsure')
