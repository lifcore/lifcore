...
/** Especialista LifPlan — Demanda. Mesmo princípio dos outros quatro: uma única chamada de raciocínio. */
export async function atenderDemandaLifplan({ demandaTexto, usuarioId, organizacaoId, clienteProspectId, imagens = [], casoIdContinuacao = null }) {
  ...
  if (casoExistente) {
    ...
    const codigoCaso = await gerarCodigoDemandaLifplan()
...
export async function encerrarConversaLifplan(casoId) {
...
export async function buscarHistoricoChatLifplan(casoId) {
  ...
  const mensagens = (eventos ?? [])
    ...
    .map((e) => {
      ...
      if (e.tipo === 'atualizacao_manual') {
        const nomeAutor = nomesPorId[e.usuario_responsavel] ?? 'Consultor'
...
async function gerarCodigoDemandaLifplan() {
  const { data, error } = await operacional.rpc('gerar_codigo_demanda_lifplan')
  if (error) {
    console.warn('Falha ao gerar código sequencial, usando fallback aleatório:', error.message)
    return `DM-LIFPLAN-${Math.floor(1000 + Math.random() * 9000)}`
  }
  return data
}
