...
/** Consulta Rápida do LiShield — mesmo motor de raciocínio único da Demanda, sem ciclo de vida. */
export async function atenderConsultaRapidaLishield({ demandaTexto, usuarioId, organizacaoId, imagens = [], consultaIdExistente = null }) {
  ...
  if (consultaIdExistente) {
    const { data } = await operacional.from('consultas_rapidas_lishield').select('*').eq('id', consultaIdExistente).maybeSingle()
    consulta = data
  }
  ...
  if (!consulta) {
    const { data: novaConsulta, error } = await operacional
      .from('consultas_rapidas_lishield')
...
/** Converte uma Consulta Rápida em Demanda de verdade, vinculando a um cliente do LiShield */
export async function vincularConsultaComoDemandaLishield({ consultaId, clienteProspectId, organizacaoId, usuarioId }) {
  const organizacaoIdFinal = organizacaoId ?? (await buscarOrganizacaoUnica())
  ...
  const { data: codigo } = await operacional.rpc('gerar_codigo_demanda_lishield')
