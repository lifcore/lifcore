...
/**
 * Especialista de Auto/Frota — Demanda. Mesmo princípio do Saúde: uma
 * única chamada de raciocínio, sem Playbooks, sem checklist obrigatório.
 */
export async function atenderDemandaAuto({ demandaTexto, usuarioId, organizacaoId, clienteProspectId, imagens = [], casoIdContinuacao = null }) {
  ...
  if (casoExistente) {
    ...
    const codigoCaso = await gerarCodigoDemandaAuto()
...
export async function encerrarConversaAuto(casoId) {
...
export async function buscarHistoricoChatAuto(casoId) {
...
/** Gera o próximo código sequencial de demanda (DM-AUTO-0001, 0002...) via função no banco */
async function gerarCodigoDemandaAuto() {
  const { data, error } = await operacional.rpc('gerar_codigo_demanda_auto')
  if (error) {
    console.warn('Falha ao gerar código sequencial, usando fallback aleatório:', error.message)
    return `DM-AUTO-${Math.floor(1000 + Math.random() * 9000)}`
  }
  return data
}
