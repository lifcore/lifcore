...
import { gerarRespostaEspecialistaLifplan } from './especialistaMotorLifplan'

/** Consulta Rápida do LifPlan — mesmo motor de raciocínio único da Demanda, sem ciclo de vida. */
export async function atenderConsultaRapidaLifplan({ demandaTexto, usuarioId, organizacaoId, imagens = [], consultaIdExistente = null }) {
  ...
  if (consultaIdExistente) {
    const { data } = await operacional.from('consultas_rapidas_lifplan').select('*').eq('id', consultaIdExistente).maybeSingle()
    consulta = data
  }
  ...
  if (!consulta) {
    const { data: novaConsulta, error } = await operacional
      .from('consultas_rapidas_lifplan')
  ...
  const resultado = await gerarRespostaEspecialistaLifplan({
    demandaTexto,
...
/** Converte uma Consulta Rápida em Demanda de verdade, vinculando a um cliente do LifPlan */
export async function vincularConsultaComoDemandaLifplan({ consultaId, clienteProspectId, organizacaoId, usuarioId }) {
  const organizacaoIdFinal = organizacaoId ?? (await buscarOrganizacaoUnica())
  ...
  const { data: codigo } = await operacional.rpc('gerar_codigo_demanda_lifplan')
