...
export default function ClienteDetailLifsurePage() {
  ...
  async function carregar() {
    const [resultado, listaApolices] = await Promise.all([
      ...
      listarApolicesLifsureDoCliente(id),
    ])
    setDados(resultado)
  ...
  return (
    <div className="cliente-detail-page" data-theme="lcds">
      <button className="cliente-voltar" onClick={() => navigate('/lifsure')}>&larr; Voltar ao pipeline</button>
      ...
      <div className="cliente-aba-conteudo">
        ...
        {abaAtiva === 'Demandas' && (
          <DemandasLifsureTab demandas={demandas} cliente={cliente} onAtualizado={carregar} />
        )}
      </div>

      {mostrarWhatsApp && (
        <WhatsAppLifsureModal
          contatoPrimario={contatoPrimario}
 ...
 * Apólice" abre o ApoliceLifsureForm de verdade; o Salvar fecha a
...
function ApolicesLifsureTab({ apolices, clienteProspectId, onAtualizado }) {
  const [mostrarForm, setMostrarForm] = useState(false)
...
function traduzirSituacaoLifsure(situacao) {
  const mapa = {
    aberto: 'Aberta',
...
function DemandasLifsureTab({ demandas, cliente, onAtualizado }) {
  const [mostrarForm, setMostrarForm] = useState(false)
  ...
  async function handleAbrirDemanda() {
    ...
    try {
      await criarDemandaManual({
        ...
        codigoRpc: 'gerar_codigo_demanda_lifsure',
      })
      setDescricao('')
      setDataAcao('')
      setMostrarForm(false)
      ...
      {mostrarForm && (
        <div className="ls-card demanda-form">
          ...
          <input
            ...
            placeholder="Ex: Cliente solicitou revisão de cobertura"
          />
              ...
              {demandasOrdenadas.map((d) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td><span className="ls-badge ls-badge-prospect">{traduzirSituacaoLifsure(d.situacao)}</span></td>
...
/** Painel de detalhe da Demanda — sem Especialista ainda (o Lifsure ainda não tem IA própria construída) */
function DemandaDetailLifsureModal({ demanda, onFechar, onAtualizado, onSalvoSemFechar }) {
  const { perfil } = useAuth()
...
function WhatsAppLifsureModal({ contatoPrimario, nomeEmpresa, apoliceRecente, onFechar }) {
  const { perfil } = useAuth()
  ...
  useEffect(() => {
    listarTemplates('lifsure').then((lista) => {
      setTemplates(lista)
      setCarregando(false)
    })
  }, [])
      ...
      <div className="ls-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {carregando ? (
          ...
          <p className="cliente-vazio-inline">
            Nenhuma mensagem padrão cadastrada ainda para o Lifsure. Cadastre em "Mensagens Padrão" no menu lateral.
          </p>
        ) : (
          <>
...
function TransferirClienteLifsureModal({ clienteId, corretorAtualId, onFechar, onTransferido }) {
