...
import { criarApoliceLifsure, atualizarApoliceLifsure } from '../../lib/crm/lifsureService'
...
export default function ApoliceLifsureForm({ clienteProspectId, apoliceExistente, onSalvo, onCancelar }) {
  ...
  useEffect(() => {
    ...
    listarProdutos({ modulo: 'lifsure' }).then(setProdutos).catch(() => {})
  }, [])
  ...
  async function handleCadastrarNovaSeguradora() {
    ...
    try {
      const nova = await criarSeguradora({ nome: nomeNovaSeguradora, categoriaSeguro: 'Lifsure' })
  ...
  async function handleSalvar() {
    ...
    try {
      ...
      if (apoliceExistente) {
        apoliceResultado = await atualizarApoliceLifsure({ apoliceId: apoliceExistente.id, clienteProspectId, dados })
      } else {
        const { data: org } = await operacional.from('organizacoes').select('id').limit(1).single()
        apoliceResultado = await criarApoliceLifsure({
  ...
  return (
    <div className="cotacao-form">
      ...
      <textarea
        ...
        rows={4}
        placeholder="Anote aqui o que for específico deste produto: LMI e franquia (RC), beneficiários e capital segurado (Vida), endereço e valor do imóvel (Patrimonial), etc."
