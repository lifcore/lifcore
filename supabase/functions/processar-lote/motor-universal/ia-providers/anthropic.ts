/**
 * MOTOR UNIVERSAL (Edge Function) — Provider Anthropic
 * Porte de scripts/motor-universal/ia-providers/anthropic.cjs.
...
async function interpretar(textoDocumento: string) {
  ...
  try {
    ...
    throw new Error(`IA não devolveu JSON válido — não vou tentar interpretar por aproximação: ${(e as Error).message}`)
  }
  ...
  return {
    ...
    eventos: (json.eventos || []).map((e: Record<string, unknown>) => ({ ...e, valor_bruto: Number(e.valor_bruto) })),
    receita_extracao: json.receita_extracao ?? null,
  }
}

export default { nome: 'anthropic', interpretar }
